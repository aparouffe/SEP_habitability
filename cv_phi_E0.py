...
for ee in e0:
    ...
    for fi in range(0,n) :
        ...
        var = metabolic_index_ST(o2,Temp,salt,zt,depth_lvl,a0,ee)
...
# plt.show()
# cc = conc[1:-1,lon2+1:-3]
fig, ax = plt.subplots(1,2,subplot_kw={'projection':ccrs.PlateCarree()},figsize=(6,3.5))
plt.subplots_adjust(top=0.9,left=0.08,right=0.95,bottom=0.25,wspace=0.3)
...
cmap = 'RdYlBu_r'  
for ni in range(2):
    m = ax[ni].contourf(lon,lat,cvs[ni][1:-1,1:-3],\
    ...
    ax[ni].scatter(281,-34,marker='*',color='red')
    ax[ni].scatter(282,-25,marker='*',color='red')
    ax[ni].scatter(251,-27,marker='*',color='red')
    ax[ni].coastlines()
    gl = ax[ni].gridlines(crs=ccrs.PlateCarree(), draw_labels=True,
                      linewidth=1, color='gray', alpha=1, linestyle='--')
    gl.xlabels_top = False
    gl.ylabels_right = False
    ax[ni].set_title(str(e0[ni])+' eV')
cbar_ax = plt.gcf().add_axes([0.25, 0.12, 0.5, 0.04])
cbar = plt.colorbar(m,pad=0.12,ax=ax[1],ticks=ticks,orientation='horizontal',cax=cbar_ax)    
...
#     plt.suptitle(str(e0[ni])+' eV')
#     plt.subplots_adjust(top=0.92,left=0.05,right=0.9,wspace=0.28,bottom=0.05)
#     # CV
#     vmin = -10; vmax = 10
#     levels = np.linspace(vmin,vmax,11)# np.arange(vmin,vmax+1,2.5)
#     ticks = np.linspace(vmin,vmax,6) #np.arange(vmin,vmax+1,5)
...
#     # ticks = [-15,-10,-5,-2,-1,0,1,2,5,10,15]#[-20,-10,-5,-1,0,1,5,10,20]
#     # bounds = [-15,-10,-5,-2,-1,0,1,2,5,10,15]
#     # norm =  BoundaryNorm(bounds, cmap_cv.N)    
#     m = ax[0].contourf(lon,lat,cvs[ni][1:-1,1:-3],\
#                         vmin=vmin,vmax=vmax,levels=levels,\
#                             cmap=cmap,extend='both',norm=norm)
...
#     m1 = ax[1].contourf(lon,lat, gS[ni][1:-1,1:-3],\
#                         vmin=vmin,vmax=vmax,levels=levels,\
...
#     cmap='coolwarm'    
#     m1 = ax[2].contourf(lon,lat, gT[ni][1:-1,1:-3],\
#                         vmin=vmin,vmax=vmax,levels=levels,\
